...
export default function RecruiterDashboard() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      console.log('✅ Recruiter profile fetched:', data)
      setProfile(data)
    } catch (error) {
      console.error('Error fetching profile:', error)
    }
  ...
  const searchCandidates = async () => {
    try {
      ...
      // Mock data for demonstration since we don't have real candidates yet
  ...
  const unlockProfile = async (candidateId: string) => {
    try {
      console.log('🔓 Unlocking profile:', candidateId)
